...
a1 = np.zeros((patch+2, patch+2))
a2 = np.zeros((patch+2, patch+2))
a3 = np.zeros((patch+2, patch+2))
a4 = np.zeros((patch+2, patch+2))
b1 = np.zeros((patch+2, patch+2))
b2 = np.zeros((patch+2, patch+2))
b3 = np.zeros((patch+2, patch+2))
b4 = np.zeros((patch+2, patch+2))
...
def pickPoint(event, x, y, flags, img):  # picking Point - when mouse clicked
    ...
    if event == cv2.EVENT_LBUTTONDBLCLK:
        ...
        if i < 5:
            cv2.rectangle(img, (x - 7, y - 7), (x + 7, y + 7), color=(0, 0, 0),
...
def saveCorner(img, x, y, i, h, w):  # save cropped corner images
    ...
    pointX, pointY = x, y
    if pointX < int((patch+2)/2):
        pointX = int((patch+2)/2)
    if pointY < int((patch+2)/2):
        pointY = int((patch+2)/2)
    if pointX + int((patch+2)/2)+1 > w:
        pointX -= int((patch+2)/2)+1
    if pointY + int((patch+2)/2)+1 > h:
        pointY -= int((patch+2)/2)+1

    if i == 1:
        a1 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(a1)
    elif i == 2:
        a2 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(a2)
    elif i == 3:
        a3 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(a3)
    elif i == 4:
        a4 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(a4)
    elif i == 5:
        b1 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(b1)
    elif i == 6:
        b2 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(b2)
    elif i == 7:
        b3 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(b3)
    elif i == 8:
        b4 = img[pointY - int((patch+2)/2):pointY + int((patch+2)/2)+1, pointX - int((patch+2)/2):pointX + int((patch+2)/2)+1].copy()
        corner.append(b4)
